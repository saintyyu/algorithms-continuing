...
public class BiTreeDistance {
     ...
     * 这里需要用到求二叉树的高度的代码。
     * 思路：如果根节点为空，则返回0；如果左孩子为空，则递归调用右孩子，如果右孩子为空，则递归调用左孩子；
     * 如果左右孩子都存在，则分别求左右子树的高度并相加，得到一个临时值temp1，
     * 递归调用其左子树并得到左子树的最大距离temp2，递归调用其右子树并得到右子树的最大距离temp3，返回temp1、temp2和temp3中的最大值返回。
     * 时间复杂度：O(n^2)
     */
    public static int calcLongestDistance(BiTree biTree) {
        ...
        return Math.max(distance, BiTreeHeight.calcBiTreeHeight(biTree.getLeft()) + BiTreeHeight.calcBiTreeHeight(biTree.getRight()) + 1);
    }
}
